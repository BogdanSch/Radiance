...
$checkRole = \WPAICG\wpaicg_roles()->user_can('wpaicg_promptbase',empty($wpaicg_action) ? 'promptbase' : $wpaicg_action);
if($checkRole){
...
<div class="wrap fs-section">
    <h2 class="nav-tab-wrapper">
        ...
        if(empty($wpaicg_action)){
            $wpaicg_action = 'promptbase';
        };
        \WPAICG\wpaicg_util_core()->wpaicg_tabs('wpaicg_promptbase',array(
            'promptbase' => esc_html__('Promptbase','gpt3-ai-content-generator'),
            'logs' => esc_html__('Logs','gpt3-ai-content-generator'),
            'settings' => esc_html__('Settings','gpt3-ai-content-generator')
        ),$wpaicg_action);
        if(!$wpaicg_action || $wpaicg_action == 'promptbase'){
            $wpaicg_action = '';
        }
        ?>
    </h2>
